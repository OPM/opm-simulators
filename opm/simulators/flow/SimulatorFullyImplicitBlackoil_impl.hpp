...
runStep(SimulatorTimer& timer)
{
    ...
    if (adaptiveTimeStepping_) {
            ...
            (const double curr_time, double substep_length, const int sub_step_number)
        {
            ...
            using TStepScalar = std::remove_cvref_t<decltype(max_next_tstep)>;

            // assume no other max is already set
            auto result = max_next_tstep < std::numeric_limits<TStepScalar>::max();
            if (result) {
                // maybe updated later
            ...
            if (schedule[reportStep].events().hasEvent(ScheduleEvents::TUNING_CHANGE)) {
                ...
                if (enableTUNING) {
                    ...
                    if (max_next_tstep < std::numeric_limits<TStepScalar>::max()) {
                        this->adaptiveTimeStepping_->updateNEXTSTEP(max_next_tstep);
                    }
                }

                result = max_next_tstep < std::numeric_limits<TStepScalar>::max();
            }

            if (schedule[reportStep].events().hasEvent(ScheduleEvents::TUNINGDP_CHANGE)) {
                // Unset the event to not trigger it again on the next sub step
                ...
                // step.
                const auto& tuning_dp = schedule[reportStep].tuning_dp();
