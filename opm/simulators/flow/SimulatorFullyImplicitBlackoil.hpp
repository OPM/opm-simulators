    ...
    bool runStep(SimulatorTimer& timer)
    {
        ...
        if (adaptiveTimeStepping_) {
            auto tuningUpdater = [enableTUNING, this,
                                  reportStep = timer.currentStepNum()](const double curr_time, double dt)
            {
                ...
                double wcycle_time_step = wcycle.nextTimeStep(curr_time,
                                                              dt,
                                                              wmatcher,
                                                              this->wellModel_().wellOpenTimes(),
                                                              this->wellModel_().wellCloseTimes(),
                                                              [this, reportStep, schedule](const std::string& name)
                                                              {
                                                                  const auto& wg_events = schedule[reportStep].wellgroup_events();
                                                                  if (wg_events.has(name)) {
                                                                      return wg_events.at(name).hasEvent(ScheduleEvents::REQUEST_OPEN_WELL);
                                                                  }
                                                                  return false;
                                                              });
            ...
            tuningUpdater(timer.simulationTimeElapsed(),
                          this->adaptiveTimeStepping_->suggestedNextStep());
