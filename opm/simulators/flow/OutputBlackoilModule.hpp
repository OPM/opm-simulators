...
public:
    ...
    {
        enum Voight { XX = 0, YY = 1, ZZ = 2, YZ = 3, XZ = 4, XY = 5, };
        ...
        if constexpr (getPropValue<TypeTag, Properties::EnableMech>()) {
            ...
            for (unsigned dofIdx = 0; dofIdx < elemCtx.numPrimaryDof(/*timeIdx=*/0); ++dofIdx) {
                ...
                this->dispX_[globalDofIdx] = disp[Voight::XX];
                this->dispY_[globalDofIdx] = disp[Voight::YY];
                this->dispZ_[globalDofIdx] = disp[Voight::ZZ];
                ...
                this->stressXX_[globalDofIdx] = stress[Voight::XX];
                this->stressYY_[globalDofIdx] = stress[Voight::YY];
                this->stressZZ_[globalDofIdx] = stress[Voight::ZZ];
                this->stressXY_[globalDofIdx] = stress[Voight::XY];
                this->stressXZ_[globalDofIdx] = stress[Voight::XZ];
                this->stressYZ_[globalDofIdx] = stress[Voight::YZ];

                const auto strain = model.strain(globalDofIdx, /*include_fracture*/true);
                this->strainXX_[globalDofIdx] = strain[Voight::XX];
                this->strainYY_[globalDofIdx] = strain[Voight::YY];
                this->strainZZ_[globalDofIdx] = strain[Voight::ZZ];
                this->strainXY_[globalDofIdx] = strain[Voight::XY];
                this->strainXZ_[globalDofIdx] = strain[Voight::XZ];
                this->strainYZ_[globalDofIdx] = strain[Voight::YZ];

                // Not including fracture
                const auto delstress = model.delstress(globalDofIdx);
                this->delstressXX_[globalDofIdx] = delstress[Voight::XX];
                this->delstressYY_[globalDofIdx] = delstress[Voight::YY];
                this->delstressZZ_[globalDofIdx] = delstress[Voight::ZZ];
                this->delstressXY_[globalDofIdx] = delstress[Voight::XY];
                this->delstressXZ_[globalDofIdx] = delstress[Voight::XZ];
                this->delstressYZ_[globalDofIdx] = delstress[Voight::YZ];

                const auto linstress = model.linstress(globalDofIdx);
                this->linstressXX_[globalDofIdx] = linstress[Voight::XX];
                this->linstressYY_[globalDofIdx] = linstress[Voight::YY];
                this->linstressZZ_[globalDofIdx] = linstress[Voight::ZZ];
                this->linstressXY_[globalDofIdx] = linstress[Voight::XY];
                this->linstressXZ_[globalDofIdx] = linstress[Voight::XZ];
                this->linstressYZ_[globalDofIdx] = linstress[Voight::YZ];

                // is the tresagii stress which make rock fracture
                const auto fracstress = model.fractureStress(globalDofIdx);
                this->fracstressXX_[globalDofIdx] = fracstress[Voight::XX];
                this->fracstressYY_[globalDofIdx] = fracstress[Voight::YY];
                this->fracstressZZ_[globalDofIdx] = fracstress[Voight::ZZ];
                this->fracstressXY_[globalDofIdx] = fracstress[Voight::XY];
                this->fracstressXZ_[globalDofIdx] = fracstress[Voight::XZ];
                this->fracstressYZ_[globalDofIdx] = fracstress[Voight::YZ];
            }
        }
    }
                ...
                if (cartesianIdx == cartesianIdxBlock) {
                             ...
                             (key.first == "BSTRSSYZ"))
                    {
                        if constexpr (HasGeoMech<RemoveCVR<decltype(problem)>>::value) {
                            ...
                            if      (key.first == "BSTRSSXX") { val.second = stress[Voight::XX]; }
                            else if (key.first == "BSTRSSYY") { val.second = stress[Voight::YY]; }
                            else if (key.first == "BSTRSSZZ") { val.second = stress[Voight::ZZ]; }
                            else if (key.first == "BSTRSSXY") { val.second = stress[Voight::XY]; }
                            else if (key.first == "BSTRSSXZ") { val.second = stress[Voight::XZ]; }
                            else    /*             BSTRSSYZ */{ val.second = stress[Voight::YZ]; }
                        }
                        else {
                            val.second = 0.0;
                        }
