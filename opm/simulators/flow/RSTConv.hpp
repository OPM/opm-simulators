...
public:
    ...
    //! \brief Obtain a const-ref to the CONV_NEW.
    const std::vector<int>& getConvNew() const
    { return conv_new_; }

    //! \brief Check if CONV is required. If so, then CONV_NEW is restarted at iteration 0.
    bool hasConv(const int iteration);
