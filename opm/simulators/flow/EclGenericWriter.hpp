...
private:
    ...
    ///                           (std::is_same_v<EquilGrid, Dune::CpGrid>).
    template<bool equilGridIsCpGrid>
    std::vector<std::unordered_map<int,int>> createCartesianToActiveMaps_(const Opm::LevelCartesianIndexMapper<EquilGrid>& levelCartMapp,
                                                                          const std::unordered_map<int,int>& cartesianToActive) const;
    /// Return level Cartesian dimensions
    ///
    /// Note: Refinement is supported only for CpGrid for now.
    /// @tparam equilGridIsCpGrid Compile-time flag indicating whether the equilGrid_ is a Dune::CpGrid
    ///                           (std::is_same_v<EquilGrid, Dune::CpGrid>).
    template<bool equilGridIsCpGrid>
    std::array<int,3> computeLevelCartDimensions_(const Opm::LevelCartesianIndexMapper<EquilGrid>& levelCartMapp,
                                                  int level) const;

    /// Return level Cartesian indices of the cells adjacent to an intersection.
    ///
    /// @tparam equilGridIsCpGrid Compile-time flag indicating whether the equilGrid_ is a Dune::CpGrid
    ///                           (std::is_same_v<EquilGrid, Dune::CpGrid>).
    template<bool equilGridIsCpGrid>
    int computeLevelCartIdx_(const Opm::LevelCartesianIndexMapper<EquilGrid>& levelCartMapp,
                             int levelCompressedIdx,
                             int level) const;
    
    /// Return level (local/compressed) indices of the cells adjacent to an intersection.
    ///
    /// @tparam equilGridIsCpGrid Compile-time flag indicating whether the equilGrid_ is a Dune::CpGrid
    ///                           (std::is_same_v<EquilGrid, Dune::CpGrid>).
    /// @tparam Intersection      Intersection between two leaf cells.
    template <bool equilGridIsCpGrid, typename Intersection>
    void computeLevelIndices_(const Intersection& intersection,
