...
#include <opm/simulators/linalg/gpuistl_hip/hypreinterface/HypreSetup.hpp>
#endif
                     ...
                     bool owner_first)
{
    ...
    if constexpr (gpuistl::is_gpu_type<MatrixType>::value) {
        return gpuistl::HypreInterface::setupSparsityPatternFromGpuMatrix(matrix, par_info, owner_first);
    } else
#endif
    {
...
#if HYPRE_USING_CUDA || HYPRE_USING_HIP
        if constexpr (gpuistl::is_gpu_type<MatrixType>::value) {
            return gpuistl::HypreInterface::computeRowIndexesWithMappingGpu(matrix, local_dune_to_local_hypre);
        } else
