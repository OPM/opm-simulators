...
public:
    ...
    OPM_HOST_DEVICE MiniVector(std::initializer_list<value_type> init)
    {
        ...
        auto n = (init.size() < Dimension) ? init.size() : Dimension;
    ...
    /**
     * @brief Apply functor `f` to each element ‑ handy for device lambdas.
     * @tparam UnaryFunc Functor accepting `(T&)`.
     */
    template <class UnaryFunc>
    OPM_HOST_DEVICE constexpr void for_each(UnaryFunc f)
    {
        for (auto& elem : data_) {
            f(elem);
        }
