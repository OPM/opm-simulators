 ...
 * @tparam M the matrix type (assumed to be DUNE::BCRSMatrix compatible)
 * @param matrix the input matrix
 * @param columnIndices [out] will be filled with column indices
 * @param rowIndices [out] will be filled with row pointers (CSR format)
 */
template <class M>
void
extractSparsityPattern(const M& matrix, std::vector<int>& columnIndices, std::vector<int>& rowIndices)
{
    ...
    for (auto& row : matrix) {
        ...
        rowIndices.push_back(to_int(columnIndices.size()));
    }
}
 ...
 * @param matrixTypeName name of the matrix type for error messages
 */
template <class M>
void
validateMatrixConversion(const M& matrix,
                         ...
                         const char* matrixTypeName)
{
    ...
    OPM_ERROR_IF(rowIndices[matrix.N()] != to_int(matrix.nonzeroes()),
                 fmt::format("Error size of rows do not sum to number of nonzeroes in {}.", matrixTypeName));
    OPM_ERROR_IF(rowIndices.size() != numberOfRows + 1,
                 fmt::format("Row indices do not match for {}.", matrixTypeName));
    OPM_ERROR_IF(columnIndices.size() != numberOfNonzeroBlocks,
                 fmt::format("Column indices do not match for {}.", matrixTypeName));
}
 ...
 * @param matrixRows number of matrix rows
 */
inline void
validateVectorMatrixSizes(size_t vectorSize, size_t matrixBlockSize, size_t matrixRows)
{
    const size_t expectedSize = matrixBlockSize * matrixRows;
