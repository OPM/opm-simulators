...
public:
    explicit AmgxError(const std::string& msg) : std::runtime_error(msg) {}
};
 ...
 */
inline std::string getAmgxErrorMessage(AMGX_RC err,
    ...
    return fmt::format("AMGX error in expression: {}\nError code: {}\nError message: {}\nLocation: {}:{} in function {}",
                       expression,
                       static_cast<int>(err),
                       amgx_err_msg,
                       file,
 ...
 */
inline void amgxSafeCall(AMGX_RC rc,
                          const std::string& expression,
                          const std::string& file,
                          const std::string& function,
                          int line) {
 ...
 * - For GPU types: Zero-copy device-to-device transfers
...
public:
    //--------------------------------------------------------------------------
    // AMGX Library Lifecycle Management
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    // AMGX Resource Creation and Management
    //--------------------------------------------------------------------------
     ...
     */
    static void destroyMatrix(AMGX_matrix_handle matrix) {
        if (matrix) {
            OPM_AMGX_SAFE_CALL(AMGX_matrix_destroy(matrix));
        }
    }
    ...
    static void updateAmgxMatrixFromGpuSparseMatrix(const GpuSparseMatrix<T>& gpuSparseMatrix, AMGX_matrix_handle amgxMatrix) {
        ...
        OPM_AMGX_SAFE_CALL(AMGX_matrix_upload_all(
            amgxMatrix,
            n,
            nnz,
            block_size,
    ...
    static void initializeMatrix(const MatrixType& matrix, AMGX_matrix_handle amgx_matrix) {
        if constexpr (is_gpu_type<MatrixType>::value) {
            ...
            OPM_AMGX_SAFE_CALL(AMGX_pin_memory(const_cast<T*>(values),
                        sizeof(T) * nnz * block_size * block_size));

            // Upload to AMGX
            OPM_AMGX_SAFE_CALL(AMGX_matrix_upload_all(amgx_matrix, N, nnz, block_size, block_size,
                                                row_ptrs.data(), col_indices.data(),
                                                const_cast<T*>(values), nullptr));
        }
    }
    ...
    /**
     * @brief Clean up matrix resources
     *
     * Handles type-specific cleanup operations (e.g., unpinning memory for CPU matrices).
     *
     * @param matrix The source matrix
     * @param amgx_matrix The AMGX matrix
     * @throws AmgxError if cleanup fails
     */
    template<typename MatrixType>
    static void cleanupMatrixResources(const MatrixType& matrix, AMGX_matrix_handle /*amgx_matrix*/) {
        if constexpr (!is_gpu_type<MatrixType>::value) {
    ...
    static void updateMatrixValues(const MatrixType& matrix, AMGX_matrix_handle amgx_matrix) {
        if constexpr (is_gpu_type<MatrixType>::value) {
            ...
            using T = typename MatrixType::field_type;
            const T* values = &(matrix[0][0][0][0]);
            OPM_AMGX_SAFE_CALL(AMGX_matrix_replace_coefficients(amgx_matrix,
                                                        matrix.N(),
                                                        matrix.nonzeroes(),
                                                        const_cast<T*>(values),
                                                        nullptr));
    ...
    static AMGX_Mode determineAmgxMode() {
        if constexpr (std::is_same_v<MatrixFieldType, double> && std::is_same_v<VectorFieldType, double>) {
            ...
            OPM_THROW(std::runtime_error, "Unsupported combination of matrix and vector types for AMGX: "
