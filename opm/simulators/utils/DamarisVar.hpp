...
class DamarisVarXMLAttributes {
  ...
  std::string
      store_; //!< Reference string to the XML attribute to indicate if data
  ...
  std::string
      select_mem_; //!< Reference string to the XML attribute select. The
  ...
  std::string
      select_file_; //!< Reference string to the XML attribute select. The
                    //!< referenced variables data is used as indices to select
                    //!< positions in the dataset file to reorder output in the
                    //!< collective HDF5 data writer (Damaris version 1.8+)
  std::string
      select_subset_; //!< Reference string to the XML attribute select. Used to
...
public:
  DamarisVarXMLAttributes() {
    ...
    mesh_ = "";
    type_ =
        "scalar"; // This is probably not needed as vector data is defined using
...
public:
  ...
  virtual void
  setDamarisParameterAndShmem(const std::vector<int> &paramSizeVal) = 0;
  virtual void setDamarisParameter(const std::vector<int> &paramSizeVal) = 0;
  virtual void
  setDamarisPosition(const std::vector<int64_t> &positionsVals) = 0;
...
template <typename T> class DamarisVar : public DamarisVarBase {
  ...
  std::vector<int>
      param_sizes_; //!< The value for any paramaters that are being used to
                    //!< describe the size of the variables data array
  std::vector<int64_t>
      positions_; //!< The offsets into the array that the data in the Variable
  ...
  std::vector<std::string>
      param_names_; //!< Contains one paramater name for each paramater that a
  ...
  DamarisVarXMLAttributes
      xml_attributes_; //!< The extra elements that need to be part of a Damaris
...
public:
  ...
  DamarisVar(int dims, std::vector<std::string> param_names,
    ...
    setDamarisParameterAndShmem(
        param_values); // Initialise the memory size in the constructor.
  }
  ...
  bool TestType(std::string variable_name) {
    ...
    if (dam_err_ != DAMARIS_OK) {
      dam_err_sstr_
          << "  ERROR rankDamarisVar::DamarisVar ()  damaris_get_type(\""
          << variable_name_
          << "\", vartype);  Damaris error = " << damaris_error_string(dam_err_)
          << std::endl;
      has_error_ = true;
      return (false);
    }
    ...
    if (vartype == DAMARIS_TYPE_DOUBLE) {
      ...
      dam_err_sstr_ << "  ERROR rank =" << rank_
                    << " : DamarisVar::DamarisVar()::  \"" << variable_name
   ...
   */
  void parameterIsSet() { parameters_set_ = true; }

  void printError(void) {
    OPM_THROW(std::runtime_error, dam_err_sstr_.str());
  }

  bool hasError(void) { return (has_error_); }
  ...
  std::string &variable_name(void) { return (variable_name_); }
  ...
  void setDamarisParameter(const std::vector<int> &paramSizeVal) {
    ...
    for (int varnum = 0; varnum < num_params_; varnum++) {
      ...
      dam_err_ = damaris_parameter_set(param_names_[varnum].c_str(),
                                       &paramSizeVal[varnum], sizeof(int));
      if (dam_err_ != DAMARIS_OK) {
        dam_err_sstr_ << "  ERROR rank =" << rank_
                      << " : class DamarisVar : damaris_parameter_set(\""
                      << param_names_[varnum]
    ...
    if (total_size > 0) {
      ...
      dam_err_sstr_ << "  ERROR rank =" << rank_
                    << " : class DamarisVar::getDataStoreBlockSize() "
  ...
  void setDamarisPosition(const std::vector<int64_t> &positionsVals) {
    ...
    dam_err_ =
        damaris_set_position(variable_name_.c_str(), positionsVals.data());
    if (dam_err_ != DAMARIS_OK) {
      dam_err_sstr_ << "  ERROR rank =" << rank_
                    << " : class DamarisVar : damaris_set_position(\""
  ...
  void setPointersToDamarisShmem(void) {
    if (parameters_set_ == true) {
      ...
      if (dam_err_ != DAMARIS_OK) {
        dam_err_sstr_ << "  ERROR rank =" << rank_
                      << " : class DamarisVar : damaris_alloc(\""
                      << variable_name_ << "\", (void **) &ret_ptr)"
                      << ", Damaris error = " << damaris_error_string(dam_err_)
                      << std::endl;
        has_error_ = true;
      }
    } else {
  ...
  void commitVariableDamarisShmem(void) {
    ...
    if (dam_err_ != DAMARIS_OK) {
      dam_err_sstr_ << "  ERROR rank =" << rank_
                    << " : class DamarisVar : damaris_commit(\""
                    << variable_name_ << "\")"
                    << ", Damaris error = " << damaris_error_string(dam_err_)
                    << std::endl;
      has_error_ = true;
  ...
  void clearVariableDamarisShmem(void) {
    ...
    if (dam_err_ != DAMARIS_OK) {
      dam_err_sstr_ << "  ERROR rank =" << rank_
                    << " : class DamarisVar : damaris_clear(\""
                    << variable_name_ << "\")"
                    << ", Damaris error = " << damaris_error_string(dam_err_)
                    << std::endl;
      has_error_ = true;
...
private:
  void formatTypeError(std::string &var_name, std::string type_name1,
                           std::string type_name2) {
    dam_err_sstr_
        << "  ERROR rank =" << rank_
        << " : DamarisVar::DamarisVar () variable_name_: \"" << var_name
        << "\" The template type of Type of DamarisVar<T> in the code: "
        << type_name1 << " does not match type in XML:" << type_name2
        << std::endl;
    has_error_ = true;
  }
}; // class DamarisVar
