...
namespace Opm
{
    ...
    computeTimeStepSize( const double dt, const int iterations, const RelativeChangeInterface& /* relativeChange */, const SimulatorTimerInterface& /* substepTimer */) const
    {
    ...
    computeTimeStepSize( const double dt, const int /* iterations */, const RelativeChangeInterface& relChange, const SimulatorTimerInterface& /*substepTimer */) const
    {
        // shift errors
        for( int i=0; i<2; ++i ) {
    ...
    computeTimeStepSize(const double dt, const int /*iterations */, const RelativeChangeInterface& relChange, const SimulatorTimerInterface& substepTimer ) const
    {
        ...
        else if (substepTimer.currentStepNum() < 3 || substepTimer.lastStepFailed() || stepFailed_)
        {
            if (substepTimer.lastStepFailed())
                stepFailed_ = true;
            else if (stepFailed_)
                stepFailed_ = false;
