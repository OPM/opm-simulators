       ...
       const std::function<bool(const int)>& wasDynamicallyClosed,  const RsConstInfo& rsConst) const
{
    ...
    for (std::size_t well_index = 0; well_index < this->size(); ++well_index) {
        ...
        // Handle RSCONST: synthesize producer gas rates as Rs * oil rate for dead-oil runs.
        // RSCONST case (oil-water, no active gas phase):
        // report producer gas as dissolved gas tied to oil production: G = Rs * O.
        if (rsConst.enabled && ws.producer && pu.phaseIsActive(oilPhaseIdx)) {
            const int oil_phase_pos = pu.canonicalToActivePhaseIdx(oilPhaseIdx);
            const Scalar oil_rate = wv[oil_phase_pos];
            well.rates.set(rt::gas, rsConst.value * oil_rate);
            well.rates.set(rt::dissolved_gas, rsConst.value * oil_rate);
