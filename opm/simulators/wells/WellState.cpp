                                            ...
                                            const WellState* prev_well_state)
{
    ...
    for (int w = 0; w < nw; ++w) {
        ...
        if (well_ecl.isMultiSegment()) {
            ...
            calculateSegmentRates(ws, segment_inlets, segment_perforations, perforation_rates, np, 0 /* top segment */, ws.segments.rates);
            ...
            // improved during the solveWellEq process
            {
                ...
                // The indices_to_combine contain the indices that are only availabe on one process, we reduce the segment pressure vector
                // later for these indices.
                std::vector<int> indices_to_combine;
                for (int seg = 1; seg < well_nseg; ++seg) {
                    // todo: this is not the ideal solution yet, but ok
                    // the problem is: segment_pressure has 25 entries but perf_press only has 5 or 7 (in total 12)
                    // -> but that is exactly what the array segment_perforations is doing, it gets the perforations for each segment, right?
                    if (!segment_perforations[seg].empty()) {
                        const int first_perf = ws.parallel_info.get().getGlobalPerfContainerFactory().globalToLocal(segment_perforations[seg][0]);
                        if (first_perf > -1) { //-1 indicates that the global id is not on this process
                            segment_pressure[seg] = perf_press[first_perf];
                        }
                        indices_to_combine.push_back(seg);
                    } else {
                ...
                // Commuinicate the segment_pressure values
                std::vector<Scalar> values_to_combine(indices_to_combine.size(), 0.0);

                for (size_t i = 0; i < indices_to_combine.size(); ++i) {
                    values_to_combine[i] = segment_pressure[indices_to_combine[i]];
                }
                ...
                for (size_t i = 0; i < indices_to_combine.size(); ++i) {
                    segment_pressure[indices_to_combine[i]] = values_to_combine[i];
                }
            }
        }
    }
...
template<class Scalar>
void WellState<Scalar>::
calculateSegmentRatesBeforeSum(const SingleWellState<Scalar>& ws,
                               ...
                               std::vector<Scalar>& segment_rates)
{
    ...
    for (const int& perf : segment_perforations[segment]) {
        auto local_perf = ws.parallel_info.get().getGlobalPerfContainerFactory().globalToLocal(perf);
        if (local_perf > -1) {
            // if local_perf == -1, then the perforation is not on this process, therefore we do not sum that here
    ...
    for (const int& inlet_seg : segment_inlets[segment]) {
        calculateSegmentRatesBeforeSum(ws, segment_inlets, segment_perforations, perforation_rates, np, inlet_seg, segment_rates);
        for (int p = 0; p < np; ++p) {
...
template<class Scalar>
void WellState<Scalar>::
calculateSegmentRates(const SingleWellState<Scalar>& ws,
                      const std::vector<std::vector<int>>& segment_inlets,
                      const std::vector<std::vector<int>>& segment_perforations,
                      const std::vector<Scalar>& perforation_rates,
                      const int np, const int segment,
                      std::vector<Scalar>& segment_rates)
{
    calculateSegmentRatesBeforeSum(ws, segment_inlets, segment_perforations, perforation_rates, np, segment, segment_rates);
    ws.parallel_info.get().communication().sum(segment_rates.data(), segment_rates.size());
}
