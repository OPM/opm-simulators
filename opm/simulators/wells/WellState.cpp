     ...
     const bool enableDistributedWells)
{
    ...
    if ((prevState != nullptr) && (prevState->size() > 0)) {
        for (int w = 0; w < nw; ++w) {
            ...
            if (prev_well.status == Well::Status::SHUT || prev_well.well_state_from_action) {
    ...
    if (prev_well_state) {
        for (int w = 0; w < nw; ++w) {
            ...
            if (prev_well_state->has(wname)) {
                ...
                if (prev_ws.status == Well::Status::SHUT || prev_ws.well_state_from_action) {
                    // Well was shut in previous state, do not
                    continue;
                }
