            ...
            for (std::size_t perf = 0; perf < completion_set.size(); ++perf) {
                ...
                if (connection.state() == Connection::State::OPEN) {
                    ...
                    if (ws.parallel_info.get().globalToLocal(perf) > -1) {
                        ...
                        std::cout << "insert into active_to_local: " << n_activeperf << " -> " << ws.parallel_info.get().globalToLocal(perf) << std::endl;
                        active_to_local.insert({n_activeperf, ws.parallel_info.get().globalToLocal(perf)});
                        n_activeperf_local++;
                    }
                    std::cout << "activeperf, perf = " << perf << ", local = " << ws.parallel_info.get().globalToLocal(perf) << ", activeperf = " << n_activeperf << ", activeperf_local = " << n_activeperf_local << std::endl;
                    n_activeperf++;
                } else {
                               ...
                               std::vector<Scalar>& segment_rates)
{
    ...
    for (const int& perf : segment_perforations[segment]) {
        ...
        if (local_perf > -1 and perforation_rates.size() > 0) {
            for (int p = 0; p < np; ++p) {
