                        ...
                        const std::vector<int>& globalCells)
{
    ...
    // overlapConnections[comm.rank()].reserve(localOverlapConnections.size());
    // for (const auto& locID : localOverlapConnections){
    //     assert(globalCells.size() > locID);
    //     overlapConnections[comm.rank()].push_back(globalCells[locID]);
    // }
    ...
    for (int i = 0; i < comm.size(); ++i) {
        ...
        connOwners[i].resize(nrConnections[i], 0);
    ...
    for (auto& rankOverlap : connOwners)
        comm.sum(rankOverlap.data(), rankOverlap.size());
    ...
    for (std::size_t i = 0; i < localOverlapConnections.size(); ++i) {
        assert(connOwners[comm.rank()][i] < comm.size());
