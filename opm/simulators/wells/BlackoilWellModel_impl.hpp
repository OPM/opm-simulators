                    ...
                    for (auto& well : this->well_container_) {
                        ...
                        if (group.hasWell(well_name)) {
                            ...
                            well->iterateWellEqWithSwitching(
                                this->simulator_, dt, inj_controls, prod_controls, this->groupStateHelper(), this->wellState(), local_deferredLogger,  false, false
                            );
            ...
            for (int i = 0; i < max_number_of_sub_iterations; i++) {
                ...
                for (const auto& well : well_container_) {
                    ...
                    if (it != this->node_pressures_.end()) {
                        ...
                        if (thp_is_limit) {
                            well->prepareWellBeforeAssembling(
                                this->simulator_, dt, this->groupStateHelper(), this->wellState(), deferred_logger
                            );
                        }
                    }
                }
