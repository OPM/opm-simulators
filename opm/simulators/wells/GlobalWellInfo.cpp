...
GlobalWellInfo<Scalar>::GlobalWellInfo(const Schedule& sched, std::size_t report_step, const std::vector<Well>& local_wells)
{
...
template<class Scalar>
bool GlobalWellInfo<Scalar>::in_injecting_group(const std::string& wname) const {
    auto global_well_index = this->name_map.at(wname);
    return this->m_in_injecting_group[global_well_index];
}

template<class Scalar>
bool GlobalWellInfo<Scalar>::in_producing_group(const std::string& wname) const {
    auto global_well_index = this->name_map.at(wname);
    return this->m_in_producing_group[global_well_index];
}

template<class Scalar>
bool GlobalWellInfo<Scalar>::is_open(const std::string& wname) const {
    auto global_well_index = this->name_map.at(wname);
    return this->m_is_open[global_well_index];
}

template<class Scalar>
void GlobalWellInfo<Scalar>::update_injector(std::size_t well_index, Well::Status well_status, Well::InjectorCMode injection_cmode) {
...
template<class Scalar>
void GlobalWellInfo<Scalar>::update_producer(std::size_t well_index, Well::Status well_status, Well::ProducerCMode production_cmode) {
    if (well_status == Well::Status::OPEN) {
        this->m_is_open[this->local_map[well_index]] = 1;
...
template<class Scalar>
void GlobalWellInfo<Scalar>::update_efficiency_scaling_factor(std::size_t well_index, const Scalar efficiency_scaling_factor) {
...
template<class Scalar>
Scalar GlobalWellInfo<Scalar>::efficiency_scaling_factor(const std::string& wname) const {
    auto global_well_index = this->name_map.at(wname);
    return this->m_efficiency_scaling_factors[global_well_index];
}

template<class Scalar>
std::size_t GlobalWellInfo<Scalar>::well_index(const std::string& wname) const {
    return this->name_map.at(wname);
}

template<class Scalar>
const std::string& GlobalWellInfo<Scalar>::well_name(std::size_t well_index) const {
