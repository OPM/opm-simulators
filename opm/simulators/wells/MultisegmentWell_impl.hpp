...
namespace Opm
{
                            ...
                            DeferredLogger& deferred_logger) const
    {
        ...
        for (int seg = 0; seg < nseg; ++seg) {
            for (const int perf : this->segments_.perforations()[seg]) {
                ...
                for(int p = 0; p < np; ++p) {
                    well_flux[this->activeCompToPhaseIdx(p)] += cq_s[p];
                }
            }
        }
                                      ...
                                      DeferredLogger& deferred_logger) const
    {
        ...
        for (int compIdx = 0; compIdx < this->num_conservation_quantities_; ++compIdx) {
            ...
            well_flux[this->activeCompToPhaseIdx(compIdx)] = rate.value();
        }
                                  ...
                                  DeferredLogger& deferred_logger) const
    {
        ...
        for (int compIdx = 0; compIdx < this->num_conservation_quantities_; ++compIdx) {
            const EvalWell rate = well_copy.primary_variables_.getQs(compIdx);
            well_potentials[this->activeCompToPhaseIdx(compIdx)] = rate.value();
        }
        debug_cost_counter_ += well_copy.debug_cost_counter_;
        return converged;
    }
                                   ...
                                   DeferredLogger& deferred_logger)
    {
        ...
        for (int seg = 0; seg < nseg; ++seg) {
            ...
            for (const int perf : this->segments_.perforations()[seg]) {
                ...
                for (int comp_idx = 0; comp_idx < this->num_conservation_quantities_; ++comp_idx) {
                    perf_rates[local_perf_index*this->number_of_phases_ + this->activeCompToPhaseIdx(comp_idx)] = cq_s[comp_idx].value();
                }
