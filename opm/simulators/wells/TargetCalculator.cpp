            ...
            DeferredLogger& deferred_logger) const
{
    ...
    case Group::InjectionCMode::VREP: {
        const std::vector<Scalar>& group_injection_reductions = this->group_state_.injection_reservoir_rates(this->group_name_);
        ...
        if (ctrl->phase != Phase::WATER)
            voidage_rate -= group_injection_reductions[pu_.phase_pos[BlackoilPhases::Aqua]];
        if (ctrl->phase != Phase::OIL)
            voidage_rate -= group_injection_reductions[pu_.phase_pos[BlackoilPhases::Liquid]];
        if (ctrl->phase != Phase::GAS)
            voidage_rate -= group_injection_reductions[pu_.phase_pos[BlackoilPhases::Vapour]];
