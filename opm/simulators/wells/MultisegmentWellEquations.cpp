...
MultisegmentWellEquations<Scalar, IndexTraits, numWellEq, numEq>::solveLegacyGlobalDSystem(const BVectorWell& rhs) const
{
    ...
    // UMFPack is double-only, so duneDSolver_ is an empty type otherwise.
    if constexpr (std::is_same_v<Scalar, double>) {
        if (!duneDSolver_) {
            OPM_THROW(std::logic_error,
        ...
        OPM_THROW(std::runtime_error,
                  "MultisegmentWell legacy global-operator support is only available "
                  "for double precision. Disable "
                  "LocalWellEqUseLegacyGlobalOperator to use the flexible "
                  "solver path instead.");
    }
#else
    OPM_THROW(std::runtime_error,
