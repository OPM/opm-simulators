...
initMatrixAndVectors()
{
    ...
    std::vector<int> cells_for_perfs_whole_well(num_perfs_whole_mswell, 0);

    for (int perf = 0; perf < num_perfs_this_process; perf++) {
        cells_for_perfs_whole_well[this->pw_info_.localToActive(perf)] = baseif_.cells_global()[perf];
    }
