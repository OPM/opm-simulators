...
namespace {
    ...
    void reapportionGasOilMixture(std::string_view                              well,
                                  ...
                                  const std::vector<Scalar>&                    previousMixture,
                                  std::vector<Scalar>&                          currentMixture,
                                  Opm::DeferredLogger&                          deferred_logger)
    {
        const auto threshold = static_cast<Scalar>(1.0e-12);

        Scalar rs {0};
        if (!props.rsmax_perf.empty() && (previousMixture[oilpos] > threshold)) {
            rs = std::min(previousMixture[gaspos] / previousMixture[oilpos], props.rsmax_perf[perf]);
        }

        Scalar rv {0};
        if (!props.rvmax_perf.empty() && (previousMixture[gaspos] > threshold)) {
            rv = std::min(previousMixture[oilpos] / previousMixture[gaspos], props.rvmax_perf[perf]);
        }
    ...
    Proceeding as if no dissolution or vaporisation for this connection)",
            ...
            if (rs > Scalar{0}) {
                ...
                currentMixture[gaspos] =
                    (previousMixture[gaspos] - previousMixture[oilpos]*rs) / denom;
            }

            if (rv > Scalar{0}) {
                // Subtract oil in gas from oil mixture
                currentMixture[oilpos] =
                    (previousMixture[oilpos] - previousMixture[gaspos]*rv) / denom;
            }
        }
    }
                                    ...
                                    Opm::DeferredLogger&                          deferred_logger)
    {
        ...
        Scalar rvw {0};
        if (!props.rvwmax_perf.empty() && (previousMixture[gaspos] > threshold)) {
            rvw = std::min(previousMixture[waterpos] / previousMixture[gaspos], props.rvwmax_perf[perf]);
        }

        Scalar rsw {0};
        if (!props.rswmax_perf.empty() && (previousMixture[waterpos] > threshold)) {
            rsw = std::min(previousMixture[gaspos] / previousMixture[waterpos], props.rswmax_perf[perf]);
        }
    ...
    Proceeding as if no dissolution or vaporisation for this connection)",
            ...
            if (rsw > Scalar{0}) {
                ...
                currentMixture[gaspos] =
                    (previousMixture[gaspos] - previousMixture[waterpos]*rsw) / d;
            }

            if (rvw > Scalar{0}) {
                // Subtract water in gas from water mixture
                currentMixture[waterpos] =
                    (previousMixture[waterpos] - previousMixture[gaspos]*rvw) / d;
            }
        }
    }
                 ...
                 DeferredLogger& deferred_logger)
{
    ...
    //    perforation.
    auto previousMixture = std::vector<Scalar>(num_comp, Scalar{0});
    auto currentMixture  = std::vector<Scalar>(num_comp, Scalar{0});
    ...
    for (int perf = 0; perf < nperf; ++perf) {
        ...
        currentMixture = previousMixture;
        ...
        if (activeGas && activeOil) {
            reapportionGasOilMixture(this->well_.name(), gaspos, oilpos, perf,
                                     props, previousMixture, currentMixture,
                                     deferred_logger);
        }
        ...
        for (auto comp = 0*num_comp; comp < num_comp; ++comp) {
            rho    += previousMixture[comp] * rho_s[comp];
            volrat += currentMixture [comp] / b    [comp];
        }
                            ...
                            std::vector<Scalar>&       previousMixture) const
{
    ...
    if (tot_surf_rate != Scalar{0}) {
        ...
        for (int component = 0; component < num_comp; ++component) {
            previousMixture[component] = std::abs(qo[component] / tot_surf_rate);
        }
    }
    else if (num_comp == 1) {
        previousMixture[num_comp - 1] = Scalar{1};
    }
    else {
        std::fill(previousMixture.begin(), previousMixture.end(), Scalar{0});

        // No flow => use fractions defined at well level for previousMixture.
        if (this->well_.isInjector()) {
            switch (this->well_.wellEcl().injectorType()) {
            case InjectorType::WATER:
                previousMixture[FluidSystem::waterCompIdx] = Scalar{1};
                break;

            case InjectorType::GAS:
                previousMixture[FluidSystem::gasCompIdx] = Scalar{1};
                break;

            case InjectorType::OIL:
                previousMixture[FluidSystem::oilCompIdx] = Scalar{1};
                break;
            ...
            if (perf == 0) {
                ...
                // preferred phase to decide the previousMixture initialization.
                ...
                // previousMixtureture from the perforation above.
                previousMixture = currentMixture;
            }
        }
    }
}
