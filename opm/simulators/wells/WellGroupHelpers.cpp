...
namespace WellGroupHelpers
{
                       ...
                       GroupState& group_state)
    {
        ...
        for (const std::string& groupName : group.groups()) {
            setCmodeGroup(
                schedule.getGroup(groupName, reportStepIdx), schedule, summaryState, reportStepIdx, group_state);
        }
                                            ...
                                            Opm::DeferredLogger& deferred_logger)
    {
        for (const std::string& groupName : group.groups()) {
            ...
            updateGuideRatesForInjectionGroups(
                                               groupTmp, schedule, summaryState, pu, reportStepIdx, wellState, group_state, guideRate, deferred_logger);
        }
        const Phase all[] = {Phase::WATER, Phase::OIL, Phase::GAS};
        for (Phase phase : all) {
                                           ...
                                           std::vector<double>& groupTargetReduction)
    {
        ...
        for (const std::string& subGroupName : group.groups()) {
            ...
            if (isInjector) {
                const Phase all[] = {Phase::WATER, Phase::OIL, Phase::GAS};
                for (Phase phase : all) {
                    Group::InjectionCMode currentGroupControl = group_state.injection_control(subGroupName, phase);
                    int phasePos;
                    if (phase == Phase::GAS && pu.phase_used[BlackoilPhases::Vapour])
                ...
                Group::ProductionCMode currentGroupControl = group_state.production_control(subGroupName);
    ...
    void updateGroupTargetReduction(const Group& group,
        ...
        std::vector<double> target_reduction(pu.num_phases);
        updateGroupTargetReduction(group,
                                   schedule,
                                   reportStepIdx,
                                   isInjector,
                                   pu,
        ...
        for (const std::string& groupName : group.groups()) {
            ...
            if (currentGroupControl == Group::InjectionCMode::FLD
                ...
                totalGuideRate += getGuideRateInj(
                                                  groupName, schedule, wellState, group_state, reportStepIdx, guideRate, target, injectionPhase, pu);
            }
        }
