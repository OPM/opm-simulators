...
public:
    ...
    virtual void addWellPressureEquations(PressureMatrix& mat,
                                          ...
                                          const SingleWellState<Scalar>& ws) const = 0;
    ...
    virtual void updateWaterThroughput(const double dt,
                                       SingleWellState<Scalar>& ws) const = 0;
    ...
    /// Modify the ws's rates if there is only one nonzero rate.
...
protected:
    ...
    virtual void checkOperabilityUnderBHPLimit(const SingleWellState<Scalar>& ws,
