                         ...
                         EvalWell& control_eq) const
{
    ...
    if (target_rate) {
        control_eq = injection_rate - target_rate->second;
    } else {
        const auto& controls = well.injectionControls(summaryState);
        control_eq = bhp - controls.bhp_limit;
    }
                            ...
                            Scalar efficiencyFactor) const
{
    ...
    return groupStateHelper.wellState().well(well_.indexOfWell()).group_target->second;
}
                          ...
                          EvalWell& control_eq) const
{
    ...
    if (target_rate) {
        ...
        control_eq = current_rate - target_rate->second;
    } else {
        const auto& controls = well.productionControls(summaryState);
        control_eq = bhp - controls.bhp_limit;
    }
                             ...
                             Scalar efficiencyFactor) const
{
    ...
    if (current_rate > 1e-14)
        scale = target_rate->second / current_rate;
