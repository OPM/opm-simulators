        ...
        if constexpr (enableBioeffects) {
            ...
            if constexpr (enableMICP) {
                ...
                source[Indices::contiUreaEqIdx] *= getPropValue<TypeTag, Properties::BlackOilUreaScalingFactor>();
                 ...
                 /*!
                 * The biofilm consists of a single microbial species and is modeled as a solid phase
                 * represented by its volume fraction, \f$\phi_b\f$. The governing equation for biofilm mass is:
                 * \f[
                 * \frac{\partial}{\partial t}(\phi_b \rho^{biofilm}) = R^{biofilm}, \quad \phi = \phi^0 - \phi_b
                 * \f]
                 * where:
                 * - \f$\rho^{biofilm}\f$: Biofilm density [kg/m³]
                 * - \f$\phi\f$: Porosity [-]
                 * - \f$\phi^0\f$: Initial porosity [-]
                 * Hydrogen consumption by the biofilm and suspended microbes is modeled using a Monod equation:
                 * \f[
                 * R^{H_2} = -\left(\phi_b \rho^{biofilm} + c_m s_l \phi\right) \mu
                 * \frac{\rho_l \chi_l^{H_2}}{K + \rho_l \chi_l^{H_2}} \frac{1}{Y}
                 * \f]
                 * where:
                 * - \f$c_m\f$: Suspended microbial concentration [kg/m³]
                 * - \f$s_l\f$: Water saturation [-]
                 * - \f$\mu\f$: Growth rate [1/s]
                 * - \f$K\f$: Half-velocity coefficient [kg/m³]
                 * - \f$Y\f$: Yield coefficient [-]
                 * - \f$\rho_l\f$: Liquid density [kg/m³]
                 * - \f$\chi_l^{H_2}\f$: Mass fraction of hydrogen in the liquid phase [-]
                 * Biofilm growth and decay is modeled as:
                 * \f[
                 * R^{biofilm} = \phi_b \rho^{biofilm} \left(
                 * \mu \frac{\rho_l \chi_l^{H_2}}{K + \rho_l \chi_l^{H_2}} - k_d - k_{str} ||\mathbf{u}_l||^\eta
                 * \right) + c_m s_l \phi k_a
                 * \f]
                 * where:
                 * - \f$k_d\f$: Decay coefficient [1/s]
                 * - \f$k_{str}\f$: Detachment coefficient [1/s]
                 * - \f$\mathbf{u}_l\f$: Liquid flux discharge per unit area [m/s]
                 * - \f$\eta\f$: Detachment exponent [-]
                 * - \f$k_a\f$: Attachment rate [1/s]
                 * Suspended microbes attach to the biofilm according to:
                 * \f[
                 * R^{microbes} = c_m s_l \phi \left(
                 * \mu \frac{\rho_l \chi_l^{H_2}}{K + \rho_l \chi_l^{H_2}} - k_d - k_a
                 * \right) + \phi_b \rho^{biofilm} k_{str} ||\mathbf{u}_l||^\eta
                 * \f]
                 */
                ...
                const Evaluation& cBiof = intQuants.biofilmConcentration() * poroRef;
                ...
                source[contiMicrobialEqIdx] += Sw * intQuants.microbialConcentration() * intQuants.porosity() *
                                               b * (- k_a - k_d) +
                                               rho_b * cBiof * k_str * pow(normVelocityCell / intQuants.porosity(), eta);
                // biofilm growth and decay rate
                source[contiBiofilmEqIdx] += (k_g - k_d - k_str * pow(normVelocityCell / intQuants.porosity(), eta)) * intQuants.biofilmConcentration() + 
                ...
                source[activeGasCompIdx] -= cBiof * rho_b * k_g / (Y * rho_gRef);
            }
        }
    }
...
public:
                                     ...
                                     unsigned timeIdx)
    {
        ...
        biofilmMass_ = referencePorosity_*biofilmConcentration_*BioeffectsModule::densityBiofilm(satnumRegionIdx);
        ...
        if constexpr (enableMICP) {
            ...
            calciteMass_ = referencePorosity_*calciteConcentration_*BioeffectsModule::densityCalcite(satnumRegionIdx);
        }
        const Evaluation poroFact = min(1.0 - (biofilmConcentration_ + calciteConcentration_) /
                                              (referencePorosity_ + 1e-8), 1.0); //phi/phi_0
