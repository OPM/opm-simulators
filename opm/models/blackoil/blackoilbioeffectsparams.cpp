...
initFromState(const EclipseState& eclState)
{
    ...
    for (unsigned pvtRegionIdx = 0; pvtRegionIdx < numPvtRegions; ++pvtRegionIdx) {
        bioDiffCoefficient_[pvtRegionIdx].resize(3);
        if (!diffMICPTables.empty()) {
            const DiffMICPTable& diffMICPTable = diffMICPTables.getTable<DiffMICPTable>(pvtRegionIdx);
            bioDiffCoefficient_[pvtRegionIdx][0] = diffMICPTable.getMicrobialDiffusion().front();
            bioDiffCoefficient_[pvtRegionIdx][1] = diffMICPTable.getOxygenDiffusion().front();
            bioDiffCoefficient_[pvtRegionIdx][2] = diffMICPTable.getUreaDiffusion().front();
        }
        else {
            bioDiffCoefficient_[pvtRegionIdx][0] = 0;
            bioDiffCoefficient_[pvtRegionIdx][1] = 0;
            bioDiffCoefficient_[pvtRegionIdx][2] = 0;
        }
    }
