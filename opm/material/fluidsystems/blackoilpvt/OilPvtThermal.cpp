...
#define INSTANCE_INT_ENERGY(...) \
    ...
    OilPvtThermal<double>::internalEnergy<__VA_ARGS__>(unsigned regionIdx, \
                                                       const __VA_ARGS__&, \
                                                       const __VA_ARGS__&, \
...
#define INSTANCE_VOL_FACTOR(...) \
    template __VA_ARGS__ \
    OilPvtThermal<double>::inverseFormationVolumeFactor<__VA_ARGS__>(unsigned regionIdx, \
                                                                     const __VA_ARGS__&, \
                                                                     const __VA_ARGS__&, \
                                                                     const __VA_ARGS__&) const;
INSTANCE_VOL_FACTOR(double)
INSTANCE_VOL_FACTOR(DenseAd::Evaluation<double,1,0u>)
INSTANCE_VOL_FACTOR(DenseAd::Evaluation<double,2,0u>)
INSTANCE_VOL_FACTOR(DenseAd::Evaluation<double,3,0u>)
INSTANCE_VOL_FACTOR(DenseAd::Evaluation<double,6,0u>)
INSTANCE_VOL_FACTOR(DenseAd::Evaluation<double,7,0u>)
INSTANCE_VOL_FACTOR(DenseAd::Evaluation<double,8,0u>)
INSTANCE_VOL_FACTOR(DenseAd::Evaluation<double,9,0u>)

#define INSTANCE_SAT_GAS(...) \
    template __VA_ARGS__ \
    OilPvtThermal<double>::saturatedGasDissolutionFactor<__VA_ARGS__>(unsigned regionIdx, \
                                                                      const __VA_ARGS__&, \
                                                                      const __VA_ARGS__&, \
                                                                      const __VA_ARGS__&, \
                                                                      const __VA_ARGS__&) const; \
    template __VA_ARGS__ \
    OilPvtThermal<double>::saturationPressure<__VA_ARGS__>(unsigned regionIdx, \
                                                           const __VA_ARGS__&, \
                                                           const __VA_ARGS__&) const; \
    template __VA_ARGS__ \
    OilPvtThermal<double>::diffusionCoefficient<__VA_ARGS__>(const __VA_ARGS__&, \
                                                             const __VA_ARGS__&, \
...
#define INSTANCE_FULL(...) \
    template __VA_ARGS__ \
    OilPvtThermal<double>::viscosity<__VA_ARGS__>(unsigned regionIdx, \
                                                  const __VA_ARGS__&, \
                                                  const __VA_ARGS__&, \
                                                  const __VA_ARGS__&) const; \
    template __VA_ARGS__ \
    OilPvtThermal<double>::saturatedViscosity<__VA_ARGS__>(unsigned regionIdx, \
                                                           const __VA_ARGS__&, \
                                                           const __VA_ARGS__&) const; \
    template __VA_ARGS__ \
    OilPvtThermal<double>::saturatedInverseFormationVolumeFactor<__VA_ARGS__>(unsigned regionIdx, \
                                                                              const __VA_ARGS__&, \
                                                                              const __VA_ARGS__&) const; \
    template __VA_ARGS__ \
    OilPvtThermal<double>::saturatedGasDissolutionFactor<__VA_ARGS__>(unsigned regionIdx, \
