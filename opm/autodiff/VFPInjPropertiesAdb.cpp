...
VFPInjPropertiesAdb::VFPInjPropertiesAdb() {

}



VFPInjPropertiesAdb::VFPInjPropertiesAdb(const VFPInjTable* table){
...
VFPInjPropertiesAdb::VFPInjPropertiesAdb(const std::map<int, VFPInjTable>& tables) {
    for (const auto& table : tables) {
        m_tables[table.first] = &table.second;
    }
}

VFPInjPropertiesAdb::ADB VFPInjPropertiesAdb::bhp(const std::vector<int>& table_id,
                                            const Wells& wells,
                                            const ADB& qs,
...
VFPInjPropertiesAdb::ADB VFPInjPropertiesAdb::bhp(const std::vector<int>& table_id,
    ...
    for (int i=0; i<nw; ++i) {
        ...
        if (table != nullptr) {
            ...
            detail::VFPEvaluationAdb bhp_val = detail::interpolate(table->getTable(), flo_i, thp_i);
...
double VFPInjPropertiesAdb::bhp(int table_id,
        const double& aqua,
        const double& liquid,
        const double& vapour,
    ...
    detail::VFPEvaluationAdb retval = detail::bhp(table, aqua, liquid, vapour, thp_arg);
    return retval.value;
}
...
double VFPInjPropertiesAdb::thp(int table_id,
        const double& aqua,
        const double& liquid,
        const double& vapour,
...
const VFPInjTable* VFPInjPropertiesAdb::getTable(const int table_id) const {
