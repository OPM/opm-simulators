...
VFPProdPropertiesAdb::VFPProdPropertiesAdb() {

}



VFPProdPropertiesAdb::VFPProdPropertiesAdb(const VFPProdTable* table){
...
VFPProdPropertiesAdb::VFPProdPropertiesAdb(const std::map<int, VFPProdTable>& tables) {
    for (const auto& table : tables) {
        m_tables[table.first] = &table.second;
    }
}

VFPProdPropertiesAdb::ADB VFPProdPropertiesAdb::bhp(const std::vector<int>& table_id,
                                              const Wells& wells,
                                              const ADB& qs,
                                              const ADB& thp_arg,
                                              const ADB& alq) const {
...
VFPProdPropertiesAdb::ADB VFPProdPropertiesAdb::bhp(const std::vector<int>& table_id,
    ...
    for (int i=0; i<nw; ++i) {
        ...
        if (table != nullptr) {
            ...
            detail::VFPEvaluationAdb bhp_val = detail::interpolate(table->getTable(), flo_i, thp_i, wfr_i, gfr_i, alq_i);
...
double VFPProdPropertiesAdb::bhp(int table_id,
        const double& aqua,
        const double& liquid,
        const double& vapour,
    ...
    detail::VFPEvaluationAdb retval = detail::bhp(table, aqua, liquid, vapour, thp_arg, alq);
    return retval.value;
}



double VFPProdPropertiesAdb::thp(int table_id,
        const double& aqua,
        const double& liquid,
        const double& vapour,
...
const VFPProdTable* VFPProdPropertiesAdb::getTable(const int table_id) const {
