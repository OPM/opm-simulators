...
BOOST_FIXTURE_TEST_CASE(ReachOverAlreadyClosedConnectionsIsNotAnEvent, Setup)
{
    // Completions 4 and 5 are already shut when a '+CON' fires on 3.  A
    // connection that is not able to flow is not closed again, so the workover
    // closes its offender and nothing else: one connection towards WPWE2, and
    // no reach.  Completions 1 and 2 keep flowing, so the topmost-completion
    // route does not apply either and WPWE3 stays silent.
    auto tracker = Opm::WellPerformanceEventTracker{};
