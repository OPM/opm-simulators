...
BOOST_AUTO_TEST_CASE(TestStandardWellInput) {
    ...
    BOOST_CHECK_THROW( StandardWell( well, -1, param, *rateConverter, 0, 3, 3, 0, 0, pdata, pinfo), std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(TestBehavoir) {
...

    {
        ...
        for (int w = 0; w < nw; ++w) {
            ...
            wells.emplace_back(new StandardWell(wells_ecl[w], current_timestep, param, *rateConverter, 0, 3, 3, w, 0, pdata, pinfo) );
        }
    }
