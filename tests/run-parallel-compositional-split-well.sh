...
# Verify that a compositional well whose connections are split across MPI
# ranks is rejected collectively with the documented diagnostic.
...
set +e
# Keep the two connections on their natural grid owners so the compositional
# model, rather than the partitioner, handles the unsupported split well.
timeout 20 \
