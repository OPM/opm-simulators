...
// Unit tests for Opm::LgrTransIndex<N> -- the key-sorted table that holds the gathered LGR
// output transmissibilities on the I/O rank. The parallel INIT regression exercises it end to
// end; these tests pin the container in isolation: empty state (the non-I/O-rank case), exact
// value round-trip, absent keys, N=3 and N=4 key widths, negative/large key components, and
// records that arrive out of key order, at several sizes.
...
BOOST_AUTO_TEST_CASE(ManyRecordsFoundWithExactValues)
{
    ...
    // A key beyond every stored key is reported as absent.
...
{
    // Negative and large components must round-trip and compare correctly.
...
BOOST_AUTO_TEST_CASE(UnsortedInputAllFound)
{
    // Gathered records arrive in rank order, not key order: here the level cycles over four
    // values, so the input is unsorted. Every key must be found and a miss reported, at any size.
    for (const int n : {1, 2, 3, 7, 8, 9, 100, 127, 128, 129, 1000}) {
        ...
        BOOST_CHECK(idx.find(Key3{0, -1, -1}) == nullptr);  // absent
    }
}
