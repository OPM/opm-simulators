...
template <typename MatrixScalar, typename VectorScalar, bool GpuInput = false>
void
testAmgxPreconditioner()
{
    if constexpr (GpuInput) {
        using Matrix = Opm::gpuistl::GpuSparseMatrix<MatrixScalar>;
        using Vector = Opm::gpuistl::GpuVector<VectorScalar>;
