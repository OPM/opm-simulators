...
public:
    ...
    AMGX_resources_handle getResources() const { return rsrc_; }
...
BOOST_AUTO_TEST_CASE(TestVectorTransfer)
{
    ...
    // Create AMGX vector
    AMGX_vector_handle amgx_vec = AmgxInterface::createVector(
...
BOOST_AUTO_TEST_CASE(TestMatrixTransfer)
{
    ...
    // Create AMGX matrix
    AMGX_matrix_handle amgx_matrix = AmgxInterface::createMatrix(
        resources.getResources(),
        AMGX_mode_dDDI
    );

    // Test full matrix transfer
    ...
    // Test coefficients-only update

    // Updating the values in the GPU matrix
    gpu_matrix.getNonZeroValues() *= 2.0;

    // Update just the coefficients of the AMGX matrix
    AmgxInterface::updateAmgxMatrixCoefficientsFromGpuSparseMatrix(gpu_matrix, amgx_matrix);

    // Clean up
    AmgxInterface::destroyMatrix(amgx_matrix);
}
...
BOOST_AUTO_TEST_CASE(TestErrorHandling)
{
    ...
    AMGX_vector_handle amgx_vec = AmgxInterface::createVector(
        resources.getResources(),
        AmgxInterface::determineAmgxMode<double, double>()
    );
