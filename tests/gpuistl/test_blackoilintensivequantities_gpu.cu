...
namespace Opm {
    // Definition to resolve linking error in test
    std::string moduleVersionName() { return "test-version"; }
}
...
struct EnableDiffusion<TypeTag, TTag::FlowGasWaterEnergyProblemTestRealDeck>
{ static constexpr bool value = false; };

template <class TypeTag>
struct EnableDispersion<TypeTag, TTag::FlowGasWaterEnergyProblemTestRealDeck>
{ static constexpr bool value = false; };
...
template <class T>
class ManagedGpuObject
{
public:
    explicit ManagedGpuObject(const T& value)
    {
        OPM_GPU_SAFE_CALL(cudaMallocManaged(&object_, sizeof(T)));
        new (object_) T(value);
    }
    ...
    ~ManagedGpuObject()
    {
        if (object_ != nullptr) {
            object_->~T();
            (void)cudaFree(object_);
        }
    }

    ManagedGpuObject(const ManagedGpuObject&) = delete;
    ManagedGpuObject& operator=(const ManagedGpuObject&) = delete;
...
static void initSimulatorOnce()
{
    ...
    AdaptiveTimeStepping<ProductionTypeTag>::registerParameters();
    registerAllParameters_<ProductionTypeTag>(/*finalizeRegistration=*/false);
                                              ...
                                              bool checkDerivatives = false)
{
    ...
    ManagedGpuObject<FluidSystemViewType> managedFluidSystemView(dynamicGpuFluidSystemView);
...
{
    using ProductionTypeTag = Opm::Properties::TTag::FlowGasWaterEnergyProblem;
    using Simulator = Opm::GetPropType<ProductionTypeTag, Opm::Properties::Simulator>;
    using PrimaryVariables =
        Opm::GetPropType<ProductionTypeTag, Opm::Properties::PrimaryVariables>;
    using IntensiveQuantities =
        Opm::GetPropType<ProductionTypeTag, Opm::Properties::IntensiveQuantities>;
    ...
    const auto& problem = sim->problem();
    const auto& solution = problem.model().solution(/*timeIdx=*/0);
    const std::size_t numCells = problem.model().numGridDof();
    BOOST_REQUIRE_EQUAL(solution.size(), numCells);

    std::vector<const PrimaryVariables*> primaryVariables(numCells);
    ...
    for (std::size_t i = 0; i < numCells; ++i) {
        ...
        intensiveQuantities[i]
            = const_cast<IntensiveQuantities*>(
                sim->model().cachedIntensiveQuantities(static_cast<unsigned>(i), 0));
        BOOST_REQUIRE(intensiveQuantities[i] != nullptr);
        for (unsigned phaseOffset = 0; phaseOffset < 2u; ++phaseOffset) {
            mobilityBefore[2u * i + phaseOffset]
                = asDouble(intensiveQuantities[i]->mobility(activePhases[phaseOffset]));
        }
...
BOOST_AUTO_TEST_CASE(TestProductionGpuDispatcherContract)
{
    const TemporaryFile tempFile("test_blackoil_dispatcher.DATA");
    {
